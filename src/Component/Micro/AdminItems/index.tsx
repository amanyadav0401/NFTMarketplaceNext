...
import { useAdminItemsStore, useMyItemsStore } from '@/utils/Zustand'
        ...
        <div className="w-[20%] md:w-[10%] flex flex-col items-center">
          <div className='relative'>
            <Image
              ...
              src={item.images?.[0].includes('https') ? item.images?.[0] : `https://${item.images?.[0]}`}
              alt="Product"
...
const AdminItemsComponent = () => {
  const { adminItems: myItems, adminItemsLoading: myItemsLoading, setAdminItems: setMyItems } = useAdminItemsStore();
  const [itemsToShow, setItemsToShow] = useState<NFTItem[]>([])
  const [loading, setLoading] = useState(false)
  ...
  const onDragEnd = (result: DropResult) => {
    ...
    const reorderedItems = Array.from(itemsToShow);
    const [removed] = reorderedItems.splice(result.source.index, 1);
    reorderedItems.splice(result.destination.index, 0, removed);

    setItemsToShow(reorderedItems);
  };

  useEffect(() => {
    if (myItems) {
      setItemsToShow(myItems)
    }
  }, [myItems])
  ...
  const saveOrderHandler = async () => {
    try {
      setLoading(true)
      await APIClient.put(`items/admin/change-order`, {
        json: {
          itemIds: itemsToShow.map(item => item._id)
        },
        headers: {
          Authorization: `Bearer ${localStorage.getItem('token')}`
        }
  ...
  return (
    <>
      <div className='w-full flex justify-end'>
        <button onClick={() => {
          saveOrderHandler()
        }} disabled={loading} className="bg-[#E377FF] disabled:opacity-50 text-white px-4 py-1 rounded-md mr-2">
          {
        ...
        <Droppable droppableId="droppable">
          {(provided) => (
            <div
              ...
              className="mt-10 p-8 overflow-x-auto whitespace-nowrap"
            >
              <div className="border-b mt-4"></div>
              {myItemsLoading && (
              ...
              {!myItemsLoading && itemsToShow?.map((item, index) => (
