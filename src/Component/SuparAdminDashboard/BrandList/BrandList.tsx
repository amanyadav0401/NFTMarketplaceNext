  ...
  return (
    <div>
      <div className="mt-5 p-6">
        ...
        {adminAnalytics?.brandUsers.map((item, index) => (
          <div key={index} className="mt-5 cursor-pointer">
            <div className="flex space-x-10 items-center">
              ...
              <div className="w-[11%]">{
                adminAnalytics?.brandNFTs.filter(nft => nft.creator as unknown === item.userId?._id).length
              }</div>
              <div className="w-[11%] text-[#E377FF]">{
                adminAnalytics?.brandNFTs.filter(nft => nft.creator as unknown === item.userId?._id).reduce((acc, nft) => acc + (nft.purchasedTokens * nft.price), 0)
              }</div>
